...
(function(){
  ...
  /* Antes llamaba a Google desde el navegador con la clave del usuario: sin
     clave, el botón del modal contestaba "Introduce tu Gemini API Key primero"
     y no pasaba nada. Ahora la redacción la hace el servidor, con el guion
     clinico compartido (sources/guion-clinico.mjs), el mismo que usan la
     portada y el chat del avatar; aquí se aporta el contexto local de Enferix
     que Javny 2.0 ya tenía recuperado. El prompt propio de esta capa se
     retira: dos guiones para el mismo boton daban dos respuestas distintas
     segun que capa atendiese. */
  async function preguntarAlServidor(q, localText){
    if(typeof window.EnferixConsultaServidor!=='function') throw new Error('La consulta al servidor no esta disponible');
    return window.EnferixConsultaServidor(q, localText?{guides:localText}:null);
  }
  ...
  function installInterceptor(){ var btn=$('qsendBtn'), inp=$('qinput'), body=$('modalBody'); if(!btn||!inp||!body||btn.dataset.in52)return; btn.dataset.in52='1'; btn.addEventListener('click',async function(ev){ var c=cfg(); if(!c.smart)return; var q=inp.value.trim(); if(!q)return; ev.preventDefault(); ev.stopImmediatePropagation(); btn.disabled=true; body.innerHTML='<div style="font-weight:700;margin-bottom:12px">'+esc(q)+'</div><div class="thinking">Javny revisa Enferix, repositorios y fuentes <span class="dots"><span></span><span></span><span></span></span></div>'; try{ var res=await askHybrid(q); body.innerHTML=renderHybrid(q,res); attachAnswerTools(q,res.answer); }catch(e){body.innerHTML='<div class="placeholder-ans">❌ Error en Javny 2.0: '+esc(e.message)+'</div>';} finally{btn.disabled=false;} },true);
  ...
  function attachAnswerTools(q,answer){
    ...
    if(ex){ex.onclick=async function(){
      ...
      try{
        var more=await preguntarAlServidor('Amplía y completa esta respuesta sin repetir lo ya dicho, cerrando los puntos que falten.\n\nPregunta original: '+q+'\n\nRespuesta anterior: '+answer, '');
  ...
  function settingsHTML(){ var c=cfg(); return '<div class="in52-grid"><div class="in52-card"><h4>🔑 Clave propia (opcional)</h4><p>Javny 2.0 ya no la necesita: la consulta la atiende el servidor con su propia clave. Esta clave sólo la usan las funciones que aún llaman al modelo desde el navegador, como el editor de Proyectos.</p><label for="in52ApiKey" class="in52-label">API Key</label> <input id="in52ApiKey" class="in52-input" type="password" placeholder="AIzaSy..." value="'+esc(apiKey())+'"><label for="in52Model" class="in52-label">Modelo</label> <input id="in52Model" class="in52-input" value="'+esc(c.model)+'"><label for="in52Backend" class="in52-label">Backend opcional de búsqueda/grounding</label> <input id="in52Backend" class="in52-input" placeholder="https://tu-servidor/search" value="'+esc(c.backendUrl||'')+'"><p>Sin backend, Javny no scrapea internet directamente por CORS/seguridad; deja repositorios como lista blanca y referencias. Con backend puedes devolver resultados reales en JSON.</p><div class="in52-row" style="margin-top:10px"><button class="in52-btn" id="in52Save">Guardar</button><button class="in52-btn secondary" id="in52Test">Probar conexión</button><button class="in52-btn danger" id="in52Clear">Borrar clave</button></div><div id="in52Status" class="in52-answer" style="display:none"></div></div><div class="in52-card"><h4>🧠 Javny 2.0</h4>'+toggle('enabled','Activar la síntesis con IA',c.enabled)+toggle('smart','Modo inteligente híbrido',c.smart)+toggle('showRefs','Mostrar referencias',c.showRefs)+toggle('repos','Repositorios clínicos permitidos',c.repos)+toggle('cache','Caché de búsquedas externas',c.cache)+toggle('safety','Aviso de seguridad clínica',c.safety)+'<div class="in52-warn">Modo híbrido: primero consulta Enferix local, después repositorios permitidos/backend, y la síntesis la redacta el servidor con el guion clínico compartido.</div></div></div><div class="in52-card" style="margin-top:12px"><h4>🌐 Repositorios fiables</h4><p>Lista blanca para evitar búsquedas genéricas poco fiables.</p><div class="in52-repos">'+c.reposList.map(function(r){return '<label class="in52-repo"><input type="checkbox" data-in52-repo="'+esc(r.id)+'" '+(r.enabled?'checked':'')+'><b>'+esc(r.name)+'</b><small>'+esc(r.url.replace(/^https?:\/\//,''))+'</small></label>';}).join('')+'</div></div><div class="in52-card" style="margin-top:12px"><h4>🧪 Prueba rápida</h4><textarea id="in52Prompt" class="in52-textarea" placeholder="Ejemplo: paciente febril, TA 85/50 y lactato elevado. ¿Actuación inicial?"></textarea><div class="in52-row" style="margin-top:10px"><button class="in52-btn" id="in52Ask">Preguntar a Javny</button></div><div id="in52Answer" class="in52-answer" style="display:none"></div></div>'; }
